...
def get_last_date(timeframe):
    if timeframe == "5m":
        last_date = pd.read_csv(f'data/indiavix_{timeframe}.csv')['timestamp'].tolist()[-1].split(' ')[0]
        print(last_date)
    else:
        last_date = pd.read_csv(f'data/indiavix_{timeframe}.csv')['timestamp'].tolist()[-1].split(' ')[0]
        print(last_date)
...
def download_history(symbol, timeframe=5):
    ...
    # df = pd.date_range(start="2023-04-01", end='2023-06-30')
    # df = pd.date_range(start="2023-07-01", end='2023-08-29')
    # df = pd.date_range(start="2023-07-01", end=datetime.today().strftime("%Y-%m-%d"))
...
def save_latest_data(symbol, timeframe):
    if timeframe == "5m":
        df1 = pd.read_csv(f'data/indiavix_{timeframe}.csv')
        df2 = download_history(symbol, timeframe)
        df = pd.concat([df1, df2], axis=0)
        df.drop_duplicates(inplace=True)
        df.to_csv(f'data/indiavix_{timeframe}.csv', index=False)
...
if __name__ == '__main__':
    save_latest_data("NSE:INDIAVIX-INDEX", "5m")
    time.sleep(10)
    save_latest_data("NSE:INDIAVIX-INDEX", "1D")
