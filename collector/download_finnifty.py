...
def get_last_date(timeframe):
    if timeframe == "5m":
        last_date = pd.read_csv(f'data/finnifty_{timeframe}.csv')['timestamp'].tolist()[-1].split(' ')[0]
        print(last_date)
    else:
        last_date = pd.read_csv(f'data/finnifty_{timeframe}.csv')['timestamp'].tolist()[-1].split(' ')[0]
        print(last_date)
...
def save_latest_data(symbol, timeframe):
    if timeframe == "5m":
        df1 = pd.read_csv(f'data/finnifty_{timeframe}.csv')
        df2 = download_history(symbol, timeframe)
        df = pd.concat([df1, df2], axis=0)
        df.to_csv(f'data/finnifty_{timeframe}.csv', index=False)
